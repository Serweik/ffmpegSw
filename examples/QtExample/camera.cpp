...
bool Camera::start() {
	...
	aVffmpegWrapper->setAudioConvertingParameters(fileDescriptor, AV_SAMPLE_FMT_S16, AV_CH_LAYOUT_STEREO, 48000);
...
void Camera::feedAudioOutput() {
	std::lock_guard<std::mutex> locker(audioSamplesMutex);
	std::cout << "feed = " << audioSamplesBuffer.size() << std::endl;
	audioSamplesBuffer.remove(0, static_cast<int>(audioDevice->write(audioSamplesBuffer)));
}
...
void Camera::audioPlaying() {
	...
	while(!audioPlayingThreadIsStopping) {
		...
		if(result > 0) {
			while(audioSamplesBuffer.size() > audioBufsize * 4) {
				std::this_thread::sleep_for(std::chrono::microseconds(interval / 2));
			}
		...
		if(!aVffmpegWrapper->endOfFile(fileDescriptor)) {
			if(!audioPlayingThreadIsRunning && aVffmpegWrapper->hasAudioStream(fileDescriptor)) {
				...
				if(audioOutput->error() != QAudio::NoError) {
					...
					connect(audioOutput, SIGNAL(notify()), this, SLOT(feedAudioOutput()));
					audioOutput->setNotifyInterval(static_cast<int>(1000.0 / (sampleRate / numSamples)));
				}
			}
